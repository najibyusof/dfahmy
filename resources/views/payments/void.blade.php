<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight">Void Payment</h2>
    </x-slot>
    ...
    <section class="max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p class="mt-4 rounded-lg bg-slate-100 px-4 py-3 text-sm text-slate-700">
            This action marks the payment as voided and excludes it from total paid calculations.
        </p>

        <form method="POST" action="{{ route('payments.void', $payment) }}" class="mt-5 space-y-3">
            @csrf
            @method('PATCH')

            <div>
                <label for="notes" class="block text-sm font-medium text-slate-700">Void Notes</label>
